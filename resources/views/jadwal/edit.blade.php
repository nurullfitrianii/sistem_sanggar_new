...
@section('title', 'Ubah Jadwal Kelas')

@section('content')
<div class="container-fluid">
    <h1 class="h4 mb-3 fw-bold text-dark">Ubah Jadwal Kelas</h1>

    <form action="{{ route('jadwal.update', $jadwal->id_jadwal) }}" method="POST" class="card border-0 shadow-sm p-4 rounded-4">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="id_program" class="form-select @error('id_program') is-invalid @enderror" required>
                ...
                @foreach($kelas as $k)
                    <option value="{{ $k->id_program }}" {{ old('id_program', $jadwal->id_program) == $k->id_program ? 'selected' : '' }}>
                        {{ $k->nama_program }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="id_pelatih" class="form-select @error('id_pelatih') is-invalid @enderror" required>
                @foreach($pelatih as $p)
                    <option value="{{ $p->id_pelatih }}" {{ old('id_pelatih', $jadwal->id_pelatih) == $p->id_pelatih ? 'selected' : '' }}>
                        {{ $p->nama_pelatih }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="id_sanggar" class="form-select @error('id_sanggar') is-invalid @enderror" required>
                @foreach($sanggar as $s)
                    <option value="{{ $s->id_sanggar }}" {{ old('id_sanggar', $jadwal->id_sanggar) == $s->id_sanggar ? 'selected' : '' }}>
                        {{ $s->nama_sanggar }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="lokasi" value="{{ old('lokasi', $jadwal->lokasi ?? 'Sanggar Goong Prasasti') }}" readonly
        ...
        <div class="mb-3">
            ...
            <textarea name="materi" rows="3" class="form-control @error('materi') is-invalid @enderror" 
                      placeholder="Contoh: Minggu ini gerak tari, minggu berikutnya gerak tangan">{{ old('materi', $jadwal->materi) }}</textarea>
        ...
        <div class="mt-3">
            <button type="submit" class="btn px-4 btn-outline-success">Simpan Perubahan</button>
